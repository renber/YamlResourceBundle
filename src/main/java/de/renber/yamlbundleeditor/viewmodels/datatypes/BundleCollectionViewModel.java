...
import de.renber.yamlbundleeditor.utils.FileTracker;
...
import de.renber.yamlbundleeditor.viewmodels.ImportViewModel;

/**
 * Represents a collection of resource bundles for different languages
 * 
 * @author renber
 *
 */
public class BundleCollectionViewModel extends DataViewModelBase<BundleCollection> {
	...
	/**
	 * Updates the revision ids of the opened files
	 */
	public void updateFileTrackIds() {
		for (BundleMetaViewModel bundle : bundles) {
			File f = new File(bundle.getModel().filePath);
			if (f.exists())
				bundle.setRevisionId(FileTracker.getRevisionId(f));
			else
				bundle.setRevisionId(null);
		}
	}

	/**
	 * Check if the currently changed files have been changed outside our
	 * application
	 */
	public boolean checkBundleFilesChanged() {
		for (BundleMetaViewModel bundle : bundles) {
			if (bundle.getRevisionId() != null) {
				File f = new File(bundle.getModel().filePath);
				if (f.exists()) {
					FileRevisionId currentId = FileTracker.getRevisionId(f);
					
					if (currentId.hasChanged(bundle.getRevisionId()))
						return true;
				}
			}
		}
	...
	private void createCommands() {
		...
		addBundleCommand = new RelayCommand(() -> {
			...
			String langCode = dialogService.showTextPrompt(loc.getString("collectionEditor:addLanguage:prompt:title"), loc.getString("collectionEditor:addLanguage:prompt:message"), "");
		...
		addResourceKeyCommand = new RelayCommand(() -> {
			...
			String value = dialogService.showTextPrompt(loc.getString("keyEditor:addKey:prompt:title"), loc.getString("keyEditor:addKey:prompt:message"), defValue, true);
	 ...
	 * 
	 * @param fromStart
	 *            true - Search all keys, false - Begin at the currently
	 *            selected key
	 */
